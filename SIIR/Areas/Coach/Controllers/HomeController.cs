...
namespace SIIR.Areas.Coach.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            Console.WriteLine($"Retrieved User ID: {userId}"); // Debugging
            if (userId == null)
            { 
            ...
            coach = _contenedorTrabajo.Coach.GetById(user.CoachId);
            if (coach == null)
            {
                return NotFound($"Coach with ID {user.CoachId} not found.");
            }
            return View(coach);
        }
